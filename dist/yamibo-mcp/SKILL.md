...
把 YamiboMCP 当作"论坛帖子检索与归档接口"来使用。
...
- 用户给出 `tid`
- 用户给出帖子 URL
- 已经从 `search_threads` 或 `browse_forum_page` 拿到候选，准备查看其中一条
...
适用场景：
- 用户问"任务好了没"
- 你刚刚创建了 `archive_thread` 或 `export_thread`
- 需要查看失败原因
...
1. 调用 `search_threads`（可指定 `forum_id`）
2. 从候选中选出最相关帖子
