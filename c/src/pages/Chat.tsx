...
export function Chat() {
  ...
  return <main className="chat-page">
    ...
    <section className="chat-workspace">
      <header><div className="chat-title-block"><span className="eyebrow">{t('chat_eyebrow')}</span>{editingTitle ? <div className="chat-title-edit"><input autoFocus value={titleDraft} onChange={e => setTitleDraft(e.target.value)} onKeyDown={e => { if (e.key === 'Enter') { e.preventDefault(); void submitRename() }; if (e.key === 'Escape') setEditingTitle(false) }} /><button className="button button-primary" disabled={!titleDraft.trim() || !!sessions.operation} onClick={() => void submitRename()}>{t('save')}</button><button className="button" disabled={!!sessions.operation} onClick={() => setEditingTitle(false)}>{t('cancel')}</button></div> : <h1>{selectedSession?.title || t('chat_title')}</h1>}</div><div className="chat-header-tools"><div className={`chat-status-pill ${ready ? 'ready' : ''}`}><span className="chat-status-dot" aria-hidden="true" />{ready ? t('chat_ready') : t('chat_not_configured')}</div>{selectedSession && !editingTitle && <div className="chat-session-menu"><button type="button" className="chat-session-menu-trigger" aria-label={t('chat_session_actions')} title={t('chat_session_actions')} aria-haspopup="menu" aria-expanded={sessionMenuOpen} onClick={() => setSessionMenuOpen(value => !value)}>⚙</button>{sessionMenuOpen && <div className="chat-session-menu-popover" role="menu"><button type="button" role="menuitem" disabled={!!sessions.operation} onClick={startRename}><span aria-hidden="true">✎</span>{t('chat_rename')}</button><button type="button" role="menuitem" className="chat-action-danger" disabled={!!sessions.operation || run.activeSessionIds.has(selectedSession.id) || !!selectedSession.active_run_id} onClick={() => void deleteSelected()}><span aria-hidden="true">⌫</span>{t('chat_delete')}</button></div>}</div>}</div></header>
