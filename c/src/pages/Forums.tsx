  ...
  return (
    <div className="catalog-page forum-status-page">
      <header className="catalog-heading">
        <h1>{lang === 'en' ? 'Archive statistics' : '归档统计'}</h1>
        <p>{lang === 'en' ? 'Archive totals, account sign-in and proxy connectivity.' : '查看板块收录量、账号签到和代理连接情况。'}</p>
        <Link to="/forum">{lang === 'en' ? 'Browse forum' : '返回论坛漫游'}</Link>
      </header>

      <section className="forum-status-section">
        <div className="forum-status-section__heading">
          <h2>{t('forum_stats')}</h2>
          <button className="btn-subtle" onClick={() => void refreshSizeCache()} disabled={refreshing}>
            {refreshing ? t('running') : t('forum_size_refresh')}
          </button>
        </div>
        {message && <div className="threads-inline-message forum-status-feedback">{message}</div>}
          ...
          <tbody>
            {forums.map(f => (
              <tr key={f.forum_id}>
                ...
                <td data-label={t('enabled')}><Badge status={f.enabled ? 'succeeded' : 'muted'}>{f.enabled ? t('yes') : t('no_label')}</Badge></td>
      ...
      <section className="forum-status-section">
        <div className="forum-status-section__heading"><h2>{t('sign_in_stats')}</h2></div>
      ...
      <section className="forum-status-section">
        ...
        {!proxyHealth ? (
          ...
          <div className="table-wrap"><table className="proxy-pool-table">
            <thead><tr><th>{t('node')}</th><th>{t('proxy_pool_status')}</th><th>{tx('百合会延时', 'Yamibo latency')}</th><th>{tx('百合会可访问', 'Yamibo reachable')}</th><th>{t('current_node')}</th></tr></thead>
            <tbody>{proxyNodes.map(node => {
              const delay = node.forum_delay_ms ?? node.delay_ms
              const reachability = node.yamibo_accessible === true ? 'ok' : node.yamibo_accessible === false ? 'error' : 'unknown'
              const reachabilityLabel = node.yamibo_accessible === true
                ? tx('可访问', 'Reachable')
                : node.yamibo_accessible === false
                  ? tx('不可访问', 'Unreachable')
                  : tx('未知', 'Unknown')
              return (
                <tr key={node.name}>
                  ...
                  <td data-label={tx('百合会延时', 'Yamibo latency')}>
                    <span className={`proxy-pool-metric proxy-pool-metric--${delay == null ? 'unknown' : reachability}`}>
                      {delay == null ? '—' : `${Math.round(delay)} ms`}
                    </span>
                  </td>
                  <td data-label={tx('百合会可访问', 'Yamibo reachable')}>
                    <span className={`proxy-pool-metric proxy-pool-metric--${reachability}`}>{reachabilityLabel}</span>
                  </td>
