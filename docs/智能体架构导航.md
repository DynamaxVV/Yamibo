...
漫画帖先供文字分析时，公开 MCP 可请求 `mode=text_only`，等待任务后读取 `capture_mode=text_only`、`image_state=not_requested` 和分页正文；需要图片时再对同一 tid 请求 `mode=full` 并等待图片回填 Job。`archive_status=complete` 只描述所请求的归档范围已完成。WebUI Chat 的受限 `create_jobs` 目前不接收 `mode`，不能把公开 MCP 的参数直接用于该工具。
...
目标分层、接口草案和迁移阶段见 [LLM 原生运行时路线图](LLM原生运行时路线图.md)。其中所有标注为规划的模块均不属于 1.1 公共契约。
