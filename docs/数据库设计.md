...
当前工作区包含待部署的 revision `016_add_jobs_started_at`、
`019_add_idle_backfill_scan_indexes`、`020_indexed_thread_list_sorts` 和 `021_thread_capture_mode`。016 增加 `jobs.started_at` 可空列；019 增加闲时扫描用的帖子与资产索引；020 补齐缺失的最近回复时间并增加帖子列表排序索引；021 增加 `threads.capture_mode`，历史记录默认 `full`。这些 revision 不重抓论坛内容，但仍属于数据库 schema 变更。仅提交 GitHub 不会改库，启动连接生产数据库的当前代码或显式执行
`yamibo-init-db` 可能应用该 revision。允许改库的发布应先备份并按部署运维指南完成迁移
验收；若某次发布不允许改库，则不要启动包含该 revision 的新镜像。
...
| capture_mode | TEXT | NOT NULL, DEFAULT 'full', CHECK(`capture_mode IN ('text_only', 'full')`) | 归档范围：`text_only` 保存楼层文字与图片引用但不下载图片；`full` 请求并保存图片 |
...
`capture_mode` 由 `021_thread_capture_mode` 引入；旧归档迁移后取默认值 `full`。`archive_status=complete` 表示当前请求的归档任务完成，和图片是否已下载须结合 `capture_mode`、资产记录与读取接口的 `image_state` 判断。
...
| idx_threads_forum_tid | (forum_id, tid) WHERE `archive_status IN ('complete', 'partial')` | 闲时扫描的版块游标批次 |
...
**索引**：
- `(tid)` — 按帖子列出资产
- `idx_assets_tid_pid_type` `(tid, pid, asset_type)` — 闲时扫描匹配楼层图片资产
...
Discussion 数据域的构建、质量阈值和查询契约见 [讨论趋势V1.md](讨论趋势V1.md)。
...
数据挖掘应从 Source/Archive 重建派生层，并记录 index/parser/model 版本；不能把 RAG chunk、topic 标签或 LLM 摘要当作不可更改的原始事实。未来 research run、lineage 和 citation 方案以 [LLM 原生运行时路线图](LLM原生运行时路线图.md) 的重新评估条件为准，当前不属于 1.0 + Phase A+ 公共契约。
...
图片资源示例：`images/floor_001_01.jpg`、`images/floor_001_02.jpg`
```
