...
当前工作区包含待部署的 revision `016_add_jobs_started_at` 和
`019_add_idle_backfill_scan_indexes`。前者增加 `jobs.started_at` 可空列；后者增加闲时扫描用的
帖子与资产索引，不重写论坛归档内容，但仍属于数据库 schema 变更。仅提交 GitHub 不会改库，启动连接生产数据库的当前代码或显式执行
