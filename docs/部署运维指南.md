...
仅提交 GitHub 不会直接修改数据库。若远端会拉取、构建并启动当前工作区代码，则可能发生数据库变更：当前工作区包含 Alembic revision `016_add_jobs_started_at` 和 `019_add_idle_backfill_scan_indexes`。019 为 `threads` 与 `assets` 创建闲时扫描索引，并更新 `alembic_version`；它不重写归档数据，但索引创建会消耗数据库 I/O 并获取表锁。
...
- 不要把包含 `016_add_jobs_started_at` 或 `019_add_idle_backfill_scan_indexes` 的工作区直接交给 NAS 自动部署。
...
`019_add_idle_backfill_scan_indexes` 为帖子和资产表增加索引。索引创建会扫描表并消耗 I/O，
需要先在备份/隔离库验证执行时间，再安排维护窗口。
...
4. **使用新镜像单独执行迁移**：
   ...
   预期版本到达 `019_add_idle_backfill_scan_indexes`。如果迁移报错、版本没有变化或出现锁超时，
...
6. **完成验收**。下面条件全部满足，才算本次发布完成：
   ...
   - `alembic_version` 已到目标 revision，`jobs.started_at` 列及 `idx_threads_forum_tid`、`idx_assets_tid_pid_type` 索引存在；
...
`016_add_jobs_started_at` 或 `019_add_idle_backfill_scan_indexes` 的新镜像。
