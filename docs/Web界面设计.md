...
│   ├── components/            # 共享组件（ThreadReader、LazyImage 等）
│   └── pages/                 # 14 个页面
...
| 功能 | 说明 |
|------|------|
| 分区切换 | 顶部标签切换分区（默认漫画区 30），选中的高亮 |
| 排序切换 | 右上角切换排序模式：默认 / 按发布时间 |
| 分页浏览 | 顶部和底部翻页控件 |
| 列表列 | TID、标题、版块、标签、发布者、发布时间、回复数、归档状态、操作 |
| 归档状态 | 已归档显示绿色徽章+"本地"链接，未归档显示灰色 badge |
| 操作 | 未归档→"归档"按钮（创建同步任务），已归档→"重新同步"按钮 |
| 响应式 | 桌面端固定列宽，移动端隐藏标签列、时间列可换行 |

### 2.14 远程贴子详情 (`/forum/:tid`) 🆕

| 功能 | 说明 |
|------|------|
| 基本信息 | 标题、发布者、时间、原贴链接 |
| 归档状态 | 已归档显示 badge + "本地详情"链接 + "重新同步"按钮 |
| 阅读器 | 复用 ThreadReader 组件（阅读预设、楼层导航、字体设置） |
| 图片 | 通过服务端代理加载，支持附件图片，正在加载时显示文件名占位 |
| 链接处理 | 帖子内 yamibo 链接自动转为内部跳转 |
| 附件清洗 | 自动过滤附件元数据文字（文件名、大小、下载次数等） |
| 分页 | 支持手动翻页（每页一页远程数据） |
