...
本文是 1.1 发布范围的唯一清单。当前操作事实仍以用户手册、部署运维指南、API 接口参考和智能体接口说明为准。

## 发布阻断项

以下项目必须在创建 1.1 tag 前全部满足：

- `pyproject.toml`、Python 包、FastAPI 和前端版本均为 `1.1.1`
...
这些项目不应阻塞 1.1，也不应以未稳定的实验接口进入 1.1 公共契约。
