...
本文是 1.2 发布范围的唯一清单。当前操作事实仍以用户手册、部署运维指南、API 接口参考和智能体接口说明为准。
...
- `image_backfill_enabled=false` 状态下完成部署基线验证；随后在隔离 PostgreSQL 或备份库验证双连接 advisory lock、候选查询最大耗时 `<3s`，再进行 dry-run、daily limit 1 的生产 canary
- canary 期间连续 20 次 `/api/threads?page=1&page_size=25` 达到 p95 `<1s`、最大值 `<3s`，且同时运行的 candidate query 始终不超过 1
...
- Web Chat 是 OpenAI-compatible 对话入口，不是内置 MCP tool-loop Agent；Hermes 仍是唯一 LLM Agent
