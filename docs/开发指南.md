...
- **闲时任务入队**：当队列空闲时，Runner 会调用 `image_backfill_scheduler.py`；调度器使用 SQLite 进程内非阻塞锁或 PostgreSQL 事务级 advisory lock，并按最多 200 个 TID 的持久化游标批次创建 `image_backfill` dry-run/apply 任务。
