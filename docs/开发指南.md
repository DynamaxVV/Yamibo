...
站内附件旧 URL 下载失败并需要刷新时，处理器在清理第一次 staging 前追加 `image.url_refresh_requested` Job 事件，记录经 `_safe_image_diagnostic()` 脱敏的失败诊断；解析新帖子后将刷新结果写入 `image_url_refresh` artifact。最终下载仍使用 `image_download_summary` 与 `image_download_diagnostics`，不能把首次旧地址的 404 当成最终图片缺失。诊断不得保存 Cookie、图片正文或带签名查询串；新增下载字段时同时检查 artifact、Job 事件和任务详情页的展示。
