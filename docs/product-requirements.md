...
面向 yamibo.com（百合会）论坛用户的本地漫画归档工具。通过 MCP（Model Context Protocol）协议，使 LLM 客户端能够浏览、搜索、归档和导出论坛漫画帖子。
...
│   │   └── exports.py          # ZIP 导出
│   ├── domain/                 # 领域模型
│   │   ├── models.py           # Job / ThreadSnapshot / TitleSnapshot / FloorSnapshot
