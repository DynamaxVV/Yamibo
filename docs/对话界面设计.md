...
> 历史规格提示：本文按 Hermes Sessions/Runs 编写，描述旧版页面及验收契约。当前本地代码使用内嵌 Pydantic AI Agent；请以[对话与知识库改造实现指南](对话与知识库改造实现指南.md)和[内置 Agent 实现与配置](内置Agent实现与配置.md)为当前实现入口，不按本文重建 Hermes 主链路。
