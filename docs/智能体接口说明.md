...
本项目面向个人 NAS 使用。归档、阅读、导出和任务恢复是核心路径；知识库、RAG、讨论趋势和研究报告属于低频可选能力。内置 Agent 与外部客户端复用同一套公开 MCP 业务能力；内置 Agent 会按需发现公开工具，并在逐项授权后执行写操作。
...
多数公开工具有对应的 CLI 命令；定时任务管理目前通过 MCP 或设置页面提供。**推荐人类操作和脚本优先使用已有 CLI；LLM 客户端优先使用 MCP tool 和 resource。**
...
| `list_scheduled_tasks` | 设置页面「定时任务」；MCP 只读 |
| `read_scheduled_task` | 设置页面「定时任务」；MCP 只读 |
| `create_archive_schedule` | 设置页面「定时任务」；MCP 写入，需审核未来周期执行的固定归档参数 |
| `set_scheduled_task_enabled` | 设置页面「定时任务」；MCP 写入，需确认当前版本 |
| `trigger_scheduled_task` | 设置页面「定时任务」；MCP 写入，需确认当前版本和触发时间 |
...
漫画帖用于文字分析时，可先调用 `create_thread_archive_job(tid=<tid>, forum_id=30, mode="text_only")`，等待 Job 完成，再通过 `read_archived_thread(view="content")` 按 `next_cursor` 读完整帖。此时 `capture_mode=text_only`、`image_state=not_requested` 表示图片引用已保留但文件未下载；即使 `archive_status=complete`，也不能称为“完整图片归档”。需要全量图片时，对同一 tid 调用 `create_thread_archive_job(mode="full")`，等待新的图片回填 Job 完成后再读取 `image_state`。仅文字归档不能直接导出 ZIP 或对单张图片使用补取接口。这里指公开 MCP 工具；WebUI Chat 受限 profile 的 `create_jobs(action, tids)` 当前没有 `mode` 参数，只能读取已有的仅文字归档，不能通过该工具创建或升级归档模式。
