...
| 路径 | 职责 | 代理修改建议 |
|------|------|-------------|
| `src/yamibo_mcp/server/cli.py` | MCP/CLI 主入口 | 入口参数和子命令改这里 |
...
| `src/yamibo_mcp/server/resources.py` | MCP resource 主入口与 guide/schema 装载 | 放本地资源和 guide 装载，不放隐藏业务逻辑 |
...
| 修改 Agent 错误契约 | `application/contracts.py`、`server/agent_adapter.py` | `uv run pytest tests/unit/test_server/test_agent_interface.py` |
...
| 范围 | 命令 |
|------|------|
| Agent 接口契约 | `uv run pytest tests/unit/test_server/test_agent_interface.py` |
