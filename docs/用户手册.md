...
`yamibo-init-db` 会创建或升级数据库 schema，仅在首次安装或已批准升级时执行。当前工作区包含 `016_add_jobs_started_at`、`019_add_idle_backfill_scan_indexes`、`020_indexed_thread_list_sorts` 和 `021_thread_capture_mode`；如果暂时不修改生产数据库，不要用本工作区启动生产服务，继续运行现有镜像或代码版本。
...
每条命令只创建或复用后台 Job；用 `wait-for-job` 或 `job-status` 核对任务完成。`text_only` 成功时帖子可显示 `archive_status=complete`，同时显示“仅文字 · 图片未保存”；这是文字归档完成，图片状态为 `not_requested`。再次请求 `full` 会对已完成的仅文字归档创建图片回填 Job，待其成功后才算完成图片升级。仅文字归档不能直接导出 ZIP；请先升级为完整归档。
...
知识库和 RAG 面向偶尔的历史研究，不是日常归档、阅读或导出的必要步骤。长时间未使用时可以保持关闭，不需要为它们单独维护任务队列。
...
- 漫画区未归档帖子提供「仅归档文字」和「归档」；前者保存全部可见楼层正文及图片引用，不下载图片
- 已仅归档文字的帖子显示「仅文字」提示和「下载图片」入口；本地详情页也可发起「下载图片，升级完整归档」
- 其他分区或普通完整归档继续使用「归档」

**查看详情**（`/forum/:tid`）：
- 显示标题、发布者、发布时间、原贴 URL
- 已归档时显示"本地详情"链接和"重新同步"按钮
- 漫画区未归档时可选「仅归档文字」或「归档」；仅文字归档完成后可点「下载图片，升级完整归档」
...
**查看帖子**：点击导航栏「帖子归档」，可搜索帖子标题；列表默认按最近回复时间从新到旧排列。
...
- 阅读预览遇到缺图时，点击缺图 URL 下方的「立即补图」，系统只为这一张图片创建 `image_backfill` 任务，不会重新归档整帖。
- 按钮会依次显示「检查中」「已排队」「下载中」；成功后页面自动刷新。同一图片已有活动任务时会直接复用，不会重复入队。
- 系统首先检查该图片原始楼层、原始序号对应的本地文件；文件有效时直接修复归档记录，不访问论坛。
- `bbs.yamibo.com/forum.php?mod=attachment...` 和站内 `/data/attachment/` 图片按配置执行重试；第三方外链通常失效后难以恢复，因此手动补取只快速尝试一次。
- 单图任务成功只表示所选图片已恢复；帖子仍可能因为其他缺图保持 `partial`。
- 在「任务」中打开失败或部分成功的补图任务，查看「图片下载诊断」：逐图展开可见 HTTP 状态、字节数与声明长度、内容格式、尝试次数、Range 续传和错误原因。「旧地址请求与地址刷新」会说明先用归档地址请求、再刷新帖子地址的过程；若刷新帖子本身失败，可在事件时间线查看 `image.url_refresh_requested`。旧任务只能显示当时已经保存的证据。
...
该页面仅用于偶尔检查或重建检索索引。它不代表原始归档事实，也不影响帖子阅读和导出。
