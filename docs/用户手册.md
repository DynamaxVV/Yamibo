...
`yamibo-init-db` 会创建或升级数据库 schema，仅在首次安装或已批准升级时执行。当前工作区包含 `016_add_jobs_started_at`、`019_add_idle_backfill_scan_indexes` 和 `020_indexed_thread_list_sorts`；如果暂时不修改生产数据库，不要用本工作区启动生产服务，继续运行现有镜像或代码版本。
