...
- selected 补图遇到旧附件地址失效并刷新帖子地址时，持久化首次下载失败的脱敏诊断、刷新结果和抓取页数；即使后续抓取帖子失败，首次失败证据仍保留在 `image.url_refresh_requested` Job 事件中。
- 任务详情增加图片下载诊断区，展示逐图 HTTP 状态、响应长度与实际字节数、格式校验、请求耗时、Range 续传、哈希和逐次尝试记录；不新增 Cookie、图片正文或带签名查询串。

### 对话、知识库与自动化

- 将 Web 对话助手定位为 Yamibo 论坛讨论助手，加入项目身份说明、按需发现工具、任务 Skills、来源回执校验，以及经用户二次确认后记录项目指导的流程。
- 打通讨论发现、原文楼层读取、引用回答、日报浏览和归档/导出操作计划；助手检索不要求先建立向量索引，写操作仍经过计划与授权边界。
- 改造知识库页面，展示讨论检索、RAG 索引能力和日报成果；增加模型连通测试与项目 Agent 指导编辑入口。
- 增加分板块日报、持久化日报规则和受限的通用定时任务；Docker 默认使用内置 Pydantic AI 调用外部 OpenAI 兼容模型，RK3588 NAS 仅运行 Yamibo 服务。
- 新增迁移 `022`–`029`，并补充讨论发现、引用、日报、规则调度、操作计划及前端界面的实现文档与测试。

## v1.3.3 (2026-09-24)

### Web UI 与访问性能

- 将侧栏“导出中心”入口替换为“归档统计”，移除归档帖子列表中的 TID 列，并调整版块统计与代理池状态布局。
- 首页最近归档优先使用 Dashboard 返回的帖子样本；切换版块时按需加载并缓存，避免首页同时请求多个帖子列表。
- 首页归档总数、导出数与版块计数合并为一次聚合查询，并延长统计缓存时间；帖子列表继续默认按最近回复时间排序。
- 普通阅览和沉浸阅读模式均显示楼层用户头像，并通过站内头像代理加载。

### 图片补取

- 选中图片补取时优先尝试归档记录中的图片地址，只有地址失效等证据明确时才重新读取帖子并刷新附件地址。
- 补图后更新资产记录与缺图状态，避免为单张图片重复读取整帖。

### 后端修复

- 修正 PostgreSQL pgvector 列维度读取；标准输入输出 MCP 模式的诊断日志写入 stderr；工作区文件缺失时返回结构化错误。

## v1.3.2 (2026-09-24)

### 帖子列表排序性能

- 首页分区列表明确按收录时间取数，与首页最终展示顺序一致，避免执行不必要的最近回复排序。
- PostgreSQL 的收录时间和最近回复时间排序改为使用线程表列及匹配索引，移除排序时逐贴查询楼层的慢路径。
- 新增迁移 `020_indexed_thread_list_sorts`，为旧帖子补齐缺失的最近回复时间，并创建分区/全局排序索引；不重新抓取论坛内容。

## v1.3.1 (2026-09-23)

### Web UI 发布补齐

- 补齐与 1.3.1 对应的前端静态构建产物，使首页版本号与后端一致。
- 部署指南说明：生产容器不应将宿主机源代码目录挂载到 `/app/src`，否则会遮住镜像内构建的 Web 静态文件；保留开发挂载时，需先在宿主机重新构建前端。

### 闲时任务与数据库查询性能

- 闲时图片补取改为扫描一批、持久化候选并逐条消费；活动任务期间不重复执行候选扫描，游标推进不会跳过同批候选。
- 合并闲时候选扫描中的重复帖子批次读取与附件聚合，并使用 `NOT EXISTS` 检查楼层图片资产。
- 为版块候选扫描和楼层图片资产匹配增加 PostgreSQL/SQLite 索引；帖子归档统计和无搜索列表计数使用短时缓存。
- 帖子归档列表默认按最近回复时间从新到旧排列。
- 每小时检查一次每日签到缓存，并降低“已检查”及调度锁竞争日志的级别，避免高频重复检查和日志刷屏。

### 数据库迁移

- 新增 `019_add_idle_backfill_scan_indexes`，部署时需按数据库迁移流程执行；GitHub 提交不会自动修改已运行数据库。

## v1.3.0 (2026-09-23)

### Web 控制台与阅读体验

- 全面更新 Web 控制台的视觉语言与页面结构，统一导航、列表、表格、操作栏和子页面标题层级，重新整理首页、任务中心、审核、日志、系列和论坛漫游等页面的信息密度。
- 调整长标题列表的可读性、列对齐和溢出处理；优化多选操作与任务进度展示，避免列表内容因操作状态发生位移。
- 改进漫画与小说沉浸阅读模式，包括浮动工具栏自动显隐、漫画显示比例档位和阅读区域边界。
- 修复楼层引用被渲染为双层引用框的问题；任务详情时间线支持按最新事件优先加载，并可继续向前翻阅历史事件。
- 更新中英文界面文案、字体、主题样式及 Web UI 设计和视觉验收文档。

### 归档与任务修复

### Job 重试与远端页面判定

- 修复 Discuz 返回“本帖已经删除，错误权限代码255”时被误判为未知反爬页面的问题；该语义结果现在归类为 THREAD_DELETED，不会继续退避重试。
- 保留非删除类的权限升级路径；只有远端传输失败、HTTP 限流或没有语义提示的未知反爬页面才进入代理轮换和 retry_later。
- 修复 Runner 使用过期 Job 快照写回失败结果导致 nodes_tried / account_ids_tried 回退的问题；终态和重试写回前读取最新 remote_attempt，历史列表按并集合并。
- Jobs API 与前端兼容读取旧任务中的 remote_attempt.prompt_text，可将历史删除帖显示为“帖子已删除”。

### 交互式单图补取

- 本地帖子阅读预览的缺图卡片新增「立即补图」，通过独立 selected `image_backfill` Job 只处理目标图片，不再为一张缺图重新同步整帖。
- 相同图片的活动任务支持幂等复用；所选图片恢复后单图 Job 独立判定成功，不受帖子其他缺图导致的整体 `partial` 状态影响。
- 图片下载改为 URL→本地路径直接映射并保留原始楼层序号，修复中间下载失败后后续图片路径错位的问题。
- 新增本地有效文件优先协调、`.part` 原子下载和 JPEG/PNG/GIF/WebP 结构完整性检查，避免把截断文件写入正式归档。
- 站内附件继续使用配置的重试预算；交互式第三方外链仅快速尝试一次，减少已永久失效外链造成的等待。
- 站内附件下载改为复用当前 `curl_cffi` 会话、Cookie、代理、浏览器指纹和 WAF Cookie；外链继续使用隔离的通用下载器。
- 图片下载失败不再丢弃异常，逐图记录 HTTP 状态、内容类型、字节数、尝试次数、耗时、错误分类和可重试性；selected 回填同步写入 Job artifacts、持久化 Job event 与结构化实时日志。
- 修复图片任务的边界分类：`HTTP 200 + image` 的真实截断响应会尝试一次 Range 续传；JPEG 结束标记后的少量服务端尾部填充不再误报为 `truncated_image`。
- 将包含 `Sorry, you have been blocked` 的响应统一归类为 `waf_response`，selected 任务错误码使用 `REMOTE_SOFT_BLOCK`，不再误报为 `IMAGE_TARGET_NOT_DOWNLOADED` 或资源不存在。
- selected 图片任务的活动任务去重改为按稳定 `target_asset_id` 判断，URL 轮换不会再造成完全重复的补图任务。
- `/api/logs` 新增 `job_id/tid/event_type/level/component/q/errors_only` 服务端过滤和时间范围元数据；`since` 正确处理 ISO-8601 时区和 epoch 秒，`errors_only` 包含部分成功/阻塞/缺失；日志页面改为增量轮询并显示 `payload/context`。
- 单图补取成功后改为原位刷新帖子和图片数据，不再整页重载，避免带 `#top` 的阅读页自动跳回顶部。

## v1.2.0 (2026-08-21)

### 图片回填与在线性能

- 将自动 `image_backfill` 调度改为 single-flight：SQLite 使用进程内非阻塞锁，PostgreSQL 使用事务级 advisory lock，避免多个 daemon worker 同时执行候选扫描。
- 将候选查询改为最多 200 个 TID 的确定性游标批次，所有楼层与资产聚合限定在当前批次，移除全局随机排序，并批量判断 blocking Job。
- 继续复用 `system_state.value_json` 保存检查时间、扫描游标和 wrap 次数；无候选路径同样受调度间隔限制，全程没有数据库迁移或 DDL。
- 帖子详情新增按 TID 查询的轻量 `active-sync-job` API，前端不再拉取 `/api/jobs?page_size=200`，消除固定 422 和不必要的大列表查询。

### 验证与发布约束

- 增加 disabled、游标推进与 wrap、三类候选原因、dry-run/apply blocking 语义、双线程 single-flight 和活动 Job 状态回归测试。
- Python 包、FastAPI 和前端版本统一推进至 `1.2.0`，更新 production bundle、README、API 参考、运维说明和发布门禁。
- 生产环境在完成隔离 PostgreSQL advisory-lock 与查询性能验证前继续保持 `image_backfill_enabled=false`。

## v1.1.1 (2026-08-19)

### Web Chat / Hermes

- 将 Web Chat 收敛到 Hermes Sessions + Runs：Session CRUD、显式历史、Run 状态机、单路 SSE 消费、断线校准和审批流程统一由新链路处理。
- 增强流式体验：用户消息立即显示，思考与工具调用按轮次合并，最终回复开始渲染时自动收起思考面板；页面关闭或切换标签页不会主动停止服务端 Run。
- 修复同一会话重复回复风险：`unknown` Run 在 Hermes 明确返回终态前继续占用 Session；历史异常的连续终态块在界面中只展示最新结果，不修改 Hermes 权威消息。
- 完善会话操作、Hermes 连接设置、API Key 脱敏占位符和 Markdown 列表/表格的窄屏渲染。

### 文档与版本

- Python 包、FastAPI 和前端版本统一推进到 `1.1.1`，同步更新锁文件、README、当前文档版本和发布门禁。
- 补充 Web Chat 的 Sessions + Runs 实现规格、真实 Hermes 验收记录与运行限制。

## v1.1.0 (2026-08-16)

### Agent 与运行稳定性

- 保留 Phase A Capability Manifest，并增加 Phase A+ Job Recovery、延迟重试和运行态诊断。
- 移除未发布的 Phase B/C 内部 Agent Runtime，Hermes 继续作为唯一 LLM Agent。
- 保留 009-012 Alembic 兼容 revision，并通过 013 清理旧 Runtime 表，兼容已升级过的数据库。

### 文档与调用体验

- 将 `docs/` 文档统一为中文命名，区分当前文档与 `历史/` 存档。
- 重写 MCP 调用者指南，明确 Capability、Job、Recovery、分页和 Resource 使用规则。
- 增加浏览器回退、反爬恢复和 PostgreSQL 迁移兼容处理。
...
- **MCP 指南补全**：创建 `docs/MCP指南/` 下 4 个指南文件（智能体工作流 / 归档模型 / 错误码 / 智能体验收速查）
