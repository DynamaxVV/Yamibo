...
def summarize_setting_effects(field_names: list[str]) -> dict[str, object]:
    targets: list[str] = []
...
def handle_settings_get(handler, settings: Settings) -> None:
    json_response(handler, _settings_payload(settings))


def handle_settings_update(handler, settings: Settings) -> None:
    body = read_json_body(handler)
    values = body.get("values")
    if not isinstance(values, dict):
        error_response(handler, "values required")
        return
    ...
    for name, value in values.items():
        ...
        if name in locked_fields:
            error_response(handler, f"{name} is overridden by environment variable")
            return
