            ...
            try:
                ...
                if isinstance(exc, RemoteFetchError) and is_http_444_error(exc):
                    if _record_444(conn):
                        ...
                        repo.fail(job.job_id, "HTTP_444", f"HTTP 444 from {exc.details.get('url', 'unknown')}; will retry with different proxy if available")
                        LOG.warning("HTTP 444 on job %s — failing single job, not yet escalating", job.job_id)
                    return DaemonResult(processed=1)
