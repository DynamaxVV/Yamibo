...
def build_mcp_server():
    ...
    @server.tool(name="browse_forum_page", description="读取漫画区某一页的帖子列表；短调用，直接返回该页帖子信息，不创建后台任务。")
    ...
    @server.tool(name="sync_forum_range", description="按漫画区页码范围抓真实帖子列表并批量创建同步任务。")
